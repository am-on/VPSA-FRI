...
typedef struct {
	    int height;		//vi�ina slike
	    int width;		//�irina slike
	    int max_gray;	//maksimalna vrednost sivin
	    int *image;		//polje pikslov na hostu
} PGMData;
//--------------------------------------------------------//
//FUNKCIJE
//Funkcija prebere sliko z imenom filename in jo zapi�e v podatkovno strukturo tipa PGMData
PGMData* readPGM(const char *filename, PGMData *data);

//Funkcija zapi�e sliko, ki se nahaja v podatkovni stukturi tipa PGMData v datoteko z imenom filename
